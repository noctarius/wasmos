...
Status: the kernel `init` process now owns the bootstrap chain: it spawns `hw-discovery`, waits until FAT answers a dedicated readiness IPC probe, and then asks the process manager to load `sysinit` from FAT. `sysinit` is reduced to late user-process startup (`chardev-client`, `cli`); config-driven startup is still pending.
...
- AssemblyScript stages a toolchain-owned root module from `lib/libc/assemblyscript/runtime.ts`; that root exports the ABI-facing `wasmos_main` entry, imports the app module, and calls the app's exported `main(args: Array<string>): i32`.
...
- The Zig shim in `lib/libc/zig/wasmos.zig` now wraps the raw host ABI behind Zig-facing `stdlib` and `fs` helpers so Zig code can use libc-style behavior without importing the C-shaped host symbols directly; the shim no longer keeps free-function compatibility aliases for console output helpers such as `putsn`, and exposes formatted output as `stdlib.printf`.
