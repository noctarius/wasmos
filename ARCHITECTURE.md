...
- A FAT12/16/32 filesystem driver runs as a kernel-hosted WASM driver, uses the `block` IPC endpoint for sector reads, and exposes the `fs` IPC endpoint (root-only `ls`/`cat`).
