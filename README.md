...
- Graphics compositor present path now accepts client shared-memory pixel buffers (`PRESENT_WINDOW arg1=shmem_id`) and shmem-backed damage lists (`arg2=damage_count`, `arg3=damage_shmem_id`); valid damage rects are blitted with clipping, and invalid/missing damage input falls back to full-frame redraw
...
- CLI `ps` memory diagnostics now split into `vm(bytes)`, `kstack(bytes)`, `heap(bytes)`, and `rss_est(bytes)` instead of one opaque memory column (`heap` aggregates wasm3 and native-driver committed heap bytes)
