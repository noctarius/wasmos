...
- WASM `libui` scaffold is now available as a shared component-tree helper (`src/libc/include/wasmos/libui.h` mirrored in `src/libsys/wasm/include/wasmos/libui.h`) with `Panel`/`Label`/`Button`/`Checkbox`/`TextInput`/`ScrollView` primitives, lightweight bitmap text rendering, app-owned IPC integration via `ui_loop_handle_ipc(...)`, and frame flushing via `ui_loop_drain(...)`.
