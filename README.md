...
- Graphics compositor present path now accepts client shared-memory pixel buffers (`PRESENT_WINDOW arg1=shmem_id`) and blits buffer-backed window content with clipping; zero `shmem_id` falls back to placeholder rectangles and damage-count is still reserved/TODO
