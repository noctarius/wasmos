...
- Device-manager policy roots are defined for udev-like evolution: `/init/devmgr/rules` (bootstrap) and `/boot/system/devmgr/rules` (runtime override). Current implementation seeds the first rule-spawned driver from bootstrap rules as ATA (`/boot/system/drivers/ata.wap`), and also uses a dedicated non-blocking reply endpoint to asynchronously read `/boot/system/devmgr/rules/default.rules` after `fs-fat` is live.
