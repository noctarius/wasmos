...
- WASM `device-manager` now uses the shared `libsys` event-loop + intent helpers on its main reply endpoint for spawn/module-meta and mount-query request/response handling, reducing ad-hoc blocking receive paths while preserving existing startup policy behavior.
