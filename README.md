...
- Graphics Phase 0 scaffold is now in-tree: shared compositor/display IPC ABI constants (`wasmos/gfx_ipc.h`), a new `gfx-compositor` service that registers `gfx`, probes framebuffer text geometry, and emits handshake marker (`[test] gfx compositor handshake ok`) when `fb` is reachable
