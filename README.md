...
- ACPI inventory-to-driver matching is active through `/boot/system/devmgr/rules/default.rules`; serial (`PNP0501`), keyboard (`PNP0303`), mouse (`PNP0F13`), and RTC (`PNP0B00`) are matched from ACPI class/subclass and spawned with scoped I/O+IRQ caps.
- RTC request/response wire format is now defined in shared headers (`src/kernel/include/rtc_ipc.h`, `src/libc/include/wasmos/rtc_ipc.h`, and `src/libsys/wasm/include/wasmos/rtc_ipc.h`) to keep driver/client payload packing consistent.
