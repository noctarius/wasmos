...
Notes:
- `make lint` expects `build/compile_commands.json`, which is generated by a normal
  `cmake -S . -B build` configure.
- The quality targets intentionally skip vendored/imported dependency trees under
  `libs/`.
- Tooling expected on `PATH` (or via env overrides): `clang-format`,
  `clang-tidy`, `gofmt`, `go`, `zig`, `rustfmt`, `rustc`, and `asc`
  (AssemblyScript, usually from `npm install`).
