...
- ring3 user-entry setup now explicitly verifies `PML4[0]` is absent after
  ...
  CPL3 entry
- default low-slot sweep mode `WASMOS_LOW_SLOT_SWEEP` now performs a boot-time
  strip+verify pass over eligible contexts and logs the first strip/verify
  failure for Phase-2 triage (`[diag] low-slot sweep ...`)
- sweep scope can be expanded with `WASMOS_LOW_SLOT_SWEEP_LEVEL`:
  `1` checks user-mode contexts only, `2` (default) also includes selected
...
- low-slot identity-map width is now tunable with `WASMOS_IDENTITY_PD_COUNT`
  (`0..4`, default `0`) for phased reduction experiments
...
- kernel memory now provides `mm_copy_from_user` / `mm_copy_to_user` helpers
  ...
  hostcalls now resolve wasm host pointers back to user VAs for explicit
...
- syscall `ipc_call` now preserves out-of-order replies in a per-process
  ...
  or re-owned endpoints instead of replying cross-context. Strict ring3 smoke
  ...
  `[test] pm status owner deny ok`. Strict ring3 smoke also injects a
  synthetic kernel-owned `PROC_IPC_SPAWN` caller and requires
