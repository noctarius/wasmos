...
- **Explorer `.wap` launches now debounce duplicate activation bursts.** Because explorer blocks in a synchronous spawn-path call, multiple already-queued double-click activations for the same row could otherwise fan out into repeated process launches after the first spawn returned. Explorer now remembers the last launched path plus scheduler tick and suppresses immediate duplicate `.wap` activation requests for a short window.
