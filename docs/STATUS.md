...
- **PM can now execute validated broker-owned `.wap` spawn plans through the existing path loader.** After `wasmos_exec_format_classify(...)` identifies a broker-owned handler, PM appends a `wasmos_broker_spawn_plan_request_t` plus handler/path metadata to the tail of its loaded FS buffer, lends that buffer to the broker read-only, and sends `PROC_BROKER_IPC_SPAWN_PLAN_REQ`. The broker reply points at a `wasmos_broker_spawn_plan_response_t` in the broker's own FS buffer; PM borrows that buffer read-only, validates the returned plan against the matched handler metadata, then reloads the returned `.wap` host path and spawns it through the normal PM path flow. Broker-supplied host args replace the original guest argv string once delegation happens. Validation: `cmake --build build --target run-kernel-unit-tests` and `cmake --build build --target run-qemu-test`.
