...
- **Subsystem dispatch is now keyed by a single explicit in-kernel handler registry.** `wasmos_app_resolve_subsystem()` no longer maps the package tag to metadata and then performs a second runtime-tag-to-ops switch. The static built-in registry now resolves each accepted 8-byte subsystem tag (`WASM`, `WASM3`, `WARP`, `WARP+JIT`, `NATIVE`) directly to one handler record that carries the resolved runtime tag plus the subsystem ops table. This keeps all implementations linked into the kernel for now, but it establishes the registration-shaped boundary needed before moving handlers behind IPC later. Validation: `cmake --build build --target run-kernel-unit-tests` and `cmake --build build --target run-qemu-test`.
