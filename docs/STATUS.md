...
- **Virtio PCI rule spawns now identify the matched device correctly across runtimes, and WARP no longer bypasses `io.port` policy.** The `virtio-net` wasm driver had been rescanning PCI config space through `0xCF8/0xCFC` even when `device-manager` had already rule-matched the NIC and spawned it with only the BAR-scoped `io.port` grant (`io_base..io_base+0x3F`). wasm3 correctly denied those config-port accesses, so the driver fell through to `[virtio-net] no device found`; WARP was masking the same policy bug because its `io_in*`/`io_out*` hostcalls did not enforce `POLICY_ACTION_IO_PORT` at all. Rule-spawned path+caps launches now preserve an optional startup-args tail, `device-manager` uses that tail to pass the matched PCI identity (`bus:slot.fn`, vendor/device, BAR I/O base, IRQ), and `virtio-net` consumes that startup identity before falling back to a direct PCI config-space probe. In parallel, WARP’s port-I/O hostcalls now mirror wasm3 capability enforcement, including `io_wait` on port `0x80`. Validation: `run-kernel-unit-tests` and `run-qemu-test` pass on the WARP build; a dedicated wasm3 build (`configs/wasm3_smp_defconfig`) now reaches the normal `run-qemu-test` halt path and advances the NIC failure from `no device found` to `init failed io=0xC000 dev=0x1000`, which reflects the separate existing wasm3 limitation that `wasmos_region_alloc` is still unimplemented for driver-owned DMA rings (`TODO(region-alloc-wasm3)` in `src/kernel/wasm3/link.c`).
