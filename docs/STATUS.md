...
- **VT AOT startup no longer burns seconds in `vt_init_ttys()`.** The long pause between `sysinit` spawning `vt` and the later `font-service` startup was not in framebuffer geometry lookup or `notify_ready`; it was the four cooperative `wasmos_sched_yield()` calls inside `vt_init_ttys()` after each TTY-grid clear. Under the current WARP AOT ring-3 path those yields could each hand off for multiple seconds, stretching VT startup by roughly 20 seconds before `start /boot/system/services/vt.wap` returned. Removing the per-TTY yields restores prompt VT ready notification and lets `sysinit` continue immediately to `font-service`, `gfx-compositor`, and the graphical apps. Validation: `run-qemu-test` passes.
