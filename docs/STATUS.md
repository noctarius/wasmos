...
- src/libui consolidation (removing duplication): the authoritative libui implementation (core generics + per-component headers owning their render/layout/popup/hit/event handlers per the earlier design) now lives in a single place at `src/libui/include/wasmos/` (seeded from the fuller libc copy that included menus + complete event extracts). The previous two copies under `src/libc/include/wasmos/` and `src/libsys/wasm/include/wasmos/` are now thin guarded forwarders (`#include` the canonical via relative paths from their wasmos/ subdir). This eliminates drift (e.g. MENU_BAR/MENU_ITEM and some handler differences) while obeying the "keep libc and libsys wrappers in sync" rule via one source. Main `CMakeLists.txt` updated to place `-I${CMAKE_SOURCE_DIR}/src/libui/include` early for WASM app compiles (and IDE target) so that both top-level `#include "wasmos/libui.h"` (from examples/apps) and inner `#include "wasmos/libui_*.h"` (from within the main header) resolve to the canonical; also extended `WASMOS_LIBC_INCLUDE_DEPS` for correct incremental rebuilds. Added `#include <stdbool.h>` and pre-include prototypes for `ui_render_component*` (and a self-proto in menu_item) in the canonical to resolve C99 implicit-decl / bool / cross-call issues exposed when components are compiled directly from the common location. libsys/wasm consumers now transparently get the full feature set. All prior component-owned dispatch (dropdown/list/text/scroll drag/menu/button/checkbox) preserved. `run-qemu-test` (full build of gfx_smoke + menu_bar + boot/halt smoke exercising libui components, events, and menus) passes.
