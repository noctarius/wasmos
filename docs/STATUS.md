...
- **Explorer rows now support double-click activation, and list-view scrollbars are visually obvious.** The new `/boot/system/utils/explorer.wap` utility no longer requires the explicit `Open Selected` button for routine navigation: repeated left-click release on the same row within a short scheduler-tick window now opens the selected directory or file entry. The underlying `libui` list view also now reserves a proper scrollbar gutter and draws a contrasting track/thumb instead of the earlier near-invisible 3-pixel marker, making overflow state discoverable and drag-scrolling much clearer.
