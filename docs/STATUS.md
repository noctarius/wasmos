...
- **Repo-local formatter/linter entrypoints now exist for the mixed-language tree.** A new top-level `Makefile` adds `make fmt`, `make fmt-check`, `make lint`, and `make quality` as the single developer-facing quality surface. The backing helper script (`scripts/quality.sh`) formats tracked repo sources for C/C++, Go, Zig, Rust, and AssemblyScript while explicitly skipping vendored dependency trees under `libs/`; its lint side uses `clang-tidy` against the configured build database, stages the TinyGo example with its shim for `go vet`, runs `zig ast-check`, compiles the Rust example with warnings denied, and compile-checks AssemblyScript sources through `asc` with the same local shim layout the build already expects. Top-level CMake now exports `compile_commands.json` by default so `make lint` has a stable `clang-tidy` database after a normal configure. Validation: `bash -n scripts/quality.sh`; `make -n quality`; `cmake -S . -B build`; `cmake --build build --target run-kernel-unit-tests`; `cmake --build build --target run-qemu-test`.
