...
- **The old `is_wasm` process flag is now narrowed to a behavior flag: `needs_runtime_lock`.** Process state no longer stores subsystem identity as a boolean. Instead, package/runtime identity stays with subsystem tags and runtime reporting, while the scheduler keeps only the behavior it actually needs: whether process entry must run under the per-process runtime lock. Sync-spawn paths no longer peek at child runtime type to force ready gating; they now rely on the same subsystem ready-policy helper used during spawn setup. Validation: `cmake --build build --target kernel`, `cmake --build build --target run-kernel-unit-tests`, and `cmake --build build --target run-qemu-test`.
