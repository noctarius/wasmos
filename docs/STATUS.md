...
- WARP JIT runtime bring-up now gets through the earlier process-manager/app-slot failure and into real user-space service startup. The WARP backend now lazily runs module `start()` before first export call (matching the wasm3 startup contract for app/service entry paths), hostcall wrappers now unwrap the runtime context through `WasmModule::getContext()` before accessing `WarpCallContext`, IPC send/last-field metadata now match the WASMOS ABI (`source`/`destination` and arg field numbering), and WARP executable-page allocation now explicitly maps higher-half aliases with EXEC permissions instead of assuming the shared 2 MiB kernel window stayed RWX after later page-table splits. Current boot baseline with WARP reaches `native-call-min`, `native-call-smoke`, `init-smoke`, `fs-manager`, `fs-init`, `device-manager`, `pci-bus`, and `acpi-bus` inventory bring-up, but the default `run-qemu-test` halt smoke still does not reach the `wamos> ` CLI prompt yet; the remaining stall is later in the storage/bootstrap/runtime chain after `device-manager` inventory completes.
