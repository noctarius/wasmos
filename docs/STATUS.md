...
- **libui menu hover previews no longer steal focus from parent menus.** Nested `UI_COMPONENT_MENU_ITEM` popups are rendered as preview-only when opened from hover, while top-level menu-bar popups and explicit click-opened child submenus still take focus. This fixes the `menu_bar` Apps menu regression where hovering `gfx_smoke` opened its window-list submenu, then moving to single-window entries like `Calculator` or `Minesweeper` no longer updated selection or activated clicks because the focused child popup trapped subsequent pointer events.
