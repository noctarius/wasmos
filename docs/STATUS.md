...
- **PM now has a read-only executable classifier that recognizes real `.wap` blobs before broker-owned format handlers.** `wasmos_exec_format_classify(...)` is the new front-door helper for executable lookup: it validates the built-in `.wap` header prefix first, then falls back to the broker handler registry for non-`.wap` inputs such as shebang scripts or jar-like blobs. The path-based PM spawn flow now calls that classifier after loading a file from FS, but still only executes the built-in `.wap` path; broker-owned matches are rejected locally with a `TODO` until the spawn-plan IPC contract exists. Validation: `cmake --build build --target run-kernel-unit-tests` and `cmake --build build --target run-qemu-test`.
