...
- **WARP ring-3 text output guard stops runaway `puts` scans from dumping adjacent binary data.** A `gdb` capture on the `gfx_smoke` serial-garbage regression showed the bad write was a real `console_write` hostcall (`hc_id=10`) with an absurd `len=11229`, not a `shmem_flush`/hostcall-index mix-up. The immediate source was the libc text path scanning past the intended banner text and into adjacent non-text bytes in linear memory. `src/libc/src/stdio.c` now bounds `puts`/`putsn` to the printable prefix of a string (ASCII text plus CR/LF/TAB) instead of streaming through the first later `'\0'` in adjacent memory. A `FIXME` marks the remaining root-cause work on the WARP ring-3 unterminated-span source. Validation: `run-kernel-unit-tests` and `run-qemu-test` pass, and `[test] gfx smoke main start` no longer emits binary serial garbage.
