...
- **SMP work distribution is gated on WARP SMP-safety; app/service workload currently stays on CPU 0.** An earlier change had process-manager unpark normal parked children onto a non-local "lightest CPU" queue (cross-CPU push placement). That destabilised boot: WARP-backed apps/services hang flakily whenever they execute on an AP. Root cause (confirmed under GDB): the scheduler never actually ran app/service work off CPU 0 before — AP work-stealing is dead because `process_spawn_idle_ap()` sets `idle_thread` but not `sched.idle`, so the steal trigger `thread == cs->idle` never fires on APs (see the `FIXME(smp-distribution)` there with the one-line fix). Once work does reach an AP (via push placement or, with that fix, work-stealing), WARP's "only one CPU inside WARP at a time" invariant is violated: `warp_runtime_enter()` only rebinds the per-CPU heap PID and takes no lock (see the `FIXME(smp-warp)` in `warp/shim.cpp`), so concurrent WARP execution corrupts shared WARP state and produces cross-CPU IPC lost-wakeups/livelocks (not deadlocks — locks stay free and CPUs churn). For now, unpark is reverted to the conservative local-enqueue path so boot is reliable; `thread->last_cpu` is updated on actual dispatch (fixes `ps`/`sched_info` CPU reporting), and `cpu_sched_online_mask()` skips not-yet-started APs. Re-enabling real distribution requires making WARP execution SMP-safe first (serialize only the JIT/native region, releasing across blocking IPC waits) and then applying the AP-steal fix. A focused scheduler self-test covers the target-CPU selection helper, including the offline-AP skip.
