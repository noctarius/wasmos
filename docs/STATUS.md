...
- **The broker-owned executable handoff is now exercised through a real user-space broker fixture.** A WASM test service self-registers its broker-backed subsystem plus bounded `.bro` executable handler through new PM IPC registration requests, receives `PROC_BROKER_IPC_SPAWN_PLAN_REQ` from PM, returns a validated `.wap` host-path plan, and a kernel smoke requester sync-spawns that broker service before exercising the delegated `PROC_IPC_SPAWN_PATH` flow against a non-`.wap` initfs file. The delegated host workload prints a unique marker with a broker-supplied argv string, proving that PM now classifies, delegates, validates, reloads, and launches the returned `.wap` workload end to end without kernel-owned broker code in the hot path. Validation: `cmake --build build --target run-kernel-unit-tests` and `cmake --build build --target run-qemu-test`.
