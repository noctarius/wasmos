...
- **Regular parked child processes now keep their spawn-time CPU placement when process-manager unparks them, while ready-gated service/driver startup stays on the conservative local release path.** The SMP scheduler was already distributing freshly spawned threads across CPUs, but most real services/apps are created as parked children and later released by process-manager. That unpark path had been re-enqueueing every child on the caller's current CPU, which is usually CPU 0, so interactive apps kept collapsing back onto the BSP even after the scheduler had picked a different target CPU at spawn time. Unpark now resolves the target CPU from the thread itself for normal parked children, preferring a valid `last_cpu`/affinity placement and otherwise falling back to the lightest allowed CPU. Explicit-ready service/driver children still use the local unpark path to avoid destabilizing boot-critical bring-up chains. A focused scheduler self-test now covers the target-selection behavior.
