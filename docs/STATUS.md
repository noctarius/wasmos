...
- **The broker-owned executable handoff now has a real boot smoke fixture, not just PM-side plumbing.** A kernel-owned stub broker process now registers a bounded `.bro` executable handler, receives `PROC_BROKER_IPC_SPAWN_PLAN_REQ` from PM, returns a validated `.wap` host-path plan, and a separate kernel requester process exercises the full `PROC_IPC_SPAWN_PATH` delegation path against a non-`.wap` initfs file. The delegated host workload prints a unique marker with a broker-supplied argv string, proving that PM now classifies, delegates, validates, reloads, and launches the returned `.wap` workload end to end. Validation: `cmake --build build --target run-kernel-unit-tests` and `cmake --build build --target run-qemu-test`.
