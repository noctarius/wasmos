...
- **PM now has a broker spawn-plan IPC contract for external executable formats.** After `wasmos_exec_format_classify(...)` identifies a broker-owned handler, PM appends a `wasmos_broker_spawn_plan_request_t` plus handler/path metadata to the tail of its loaded FS buffer, lends that buffer to the broker read-only, and sends `PROC_BROKER_IPC_SPAWN_PLAN_REQ`. The broker reply points at a `wasmos_broker_spawn_plan_response_t` in the broker's own FS buffer; PM borrows that buffer read-only and validates the returned plan against the matched handler metadata. This step intentionally stops at the validated-plan boundary for now: only built-in `.wap` execution exists today, so a successful broker plan currently returns a distinct deferred error instead of launching. Validation: `cmake --build build --target run-kernel-unit-tests` and `cmake --build build --target run-qemu-test`.
