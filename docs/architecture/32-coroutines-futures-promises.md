...
| `sys_wait_u32` / `sys_wake_u32` for worker parking (§14, §37)         | `futex_wait(uaddr, expected, timeout_ms, ctx)` / `futex_wake(uaddr, count, ctx)` (`futex.c`), hostcall ids 16/17, keyed by **physical** address (cross-process via a shared transfer buffer), built on `sched_event_t`.                                                                                                                                                                                                                                                                                                    | ⚠️ **Exists but not surfaced.** No `wasmos_futex_*` in libc `api.h`; **absent from the native `int 0x80` syscall path** (`syscall.c` has no futex). Must be wired up before §14 works. `mutex.h:50` even carries a TODO to consume it.                                                                                                                |
