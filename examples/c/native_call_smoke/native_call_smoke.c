...
int main(int argc, char** argv) {
...

    {
        ...
        int32_t call_rc = -1;
        if (endpoint >= 0) {
            call_rc = wasmos_ipc_send(endpoint, endpoint, 0x4242, request_id, 0xDEAD, 0xBEEF, 0, 0);
        }
        if (call_rc == 0) {
        ...
        if (endpoint < 0 || call_rc < 0 || reply.type != 0x4242 || reply.request_id != request_id ||
            reply.arg0 != 0xDEAD || reply.arg1 != 0xBEEF) {
