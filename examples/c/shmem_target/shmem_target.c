...
static int wait_for_stage(int32_t ep, shmem_sync_t* s) {
    if (wasmos_ipc_select_one(ep) < 0)
        return -1;
    return sync_read(s);
}
...
int main(int argc, char** argv) {
    ...
    sync.stage = 1;
    if (sync_write(&sync) != 0) {
        puts("[test] shmem e2e target write-stage1-failed");
        return 1;
    }
    if (wasmos_ipc_send(owner_ep, my_ep, 1, 0, 0, 0, 0, 0) != 0) {
        puts("[test] shmem e2e target send-stage1-failed");
        return 1;
    }

    /* Stage 2: owner has granted the shmem. */
    if (wait_for_stage(my_ep, &sync) != 0 || sync.stage != 2) {
        puts("[test] shmem e2e target no-stage2");
        return 1;
    }
    ...
    if (wasmos_shmem_unmap(sync.shmem_id) != 0) {
        puts("[test] shmem e2e grant unmap failed");
        return 1;
    }
    ...
    sync.stage = 3;
    if (sync_write(&sync) != 0) {
        puts("[test] shmem e2e target write-stage3-failed");
        return 1;
    }
    if (wasmos_ipc_send(owner_ep, my_ep, 3, 0, 0, 0, 0, 0) != 0) {
        puts("[test] shmem e2e target send-stage3-failed");
        return 1;
    }

    /* Stage 4: owner has revoked the grant. */
    if (wait_for_stage(my_ep, &sync) != 0 || sync.stage != 4) {
        puts("[test] shmem e2e target no-stage4");
        return 1;
    }
    ...
    sync.stage = 91;
    if (sync_write(&sync) != 0) {
        puts("[test] shmem e2e target write-stage91-failed");
        return 1;
    }
    if (wasmos_ipc_send(owner_ep, my_ep, 91, 0, 0, 0, 0, 0) != 0) {
        puts("[test] shmem e2e target send-stage91-failed");
        return 1;
    }
