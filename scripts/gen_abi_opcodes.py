...
def emit_rpc_as(m):
    ...
    future/coroutine bridge, so — unlike the C stubs, which return a future over
    the libsys ipc-future bridge — these are BLOCKING wrappers over ipc.call (the
    idiom date.ts uses). Blocking IPC is the least-preferred rung (coroutines >
    push > blocking), but the only one AS supports today. Drop into an AS app
    project alongside wasmos.ts + wasmos_opcodes.ts."""
    ops = _rpc_ops(m)
    imports = sorted(
