...
def main() -> int:
    ...
    p.add_argument("--source-dir", required=True, help="CMake source root (-S)")
    p.add_argument("--build-dir",  required=True, help="Shadow build directory (-B)")
    p.add_argument("--clang",      required=True, help="Clang compiler path")
    p.add_argument("--lld",        required=True, help="LLD linker name or path")
    p.add_argument("--objcopy",    required=True, help="llvm-objcopy path")
    p.add_argument("--ovmf-code",  required=True, help="OVMF_CODE.fd path")
    ...
    p.add_argument("--target",     required=True, help="CMake target to build in shadow tree")
    args = p.parse_args()
    ...
    build_cmd = [
        args.cmake, "--build", args.build_dir, "--target", args.target,
    ]

    for cmd in (configure_cmd, build_cmd):
        rc = subprocess.call(cmd, cwd=args.source_dir)
        if rc != 0:
            return rc
