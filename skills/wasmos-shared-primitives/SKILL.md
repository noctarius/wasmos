...
|---|---|
| `wasmos_xfer_buffer_*`, `wasmos_buffer_borrow`, `wasmos_block_buffer_*`, `wasmos_dma_*_borrow` — and the bare `xfer_buffer_*` / `dma_*_borrow` externs in the generated Zig and Rust bindings, and the Zig wrappers `bufferAcquire` / `bufferBorrow` / `bufferRead` / `bufferWrite` / `bufferRelease` | `docs/architecture/12-dma-transfers.md` §"The object / owner / borrow model (READ THIS FIRST)" |
