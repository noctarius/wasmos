...
fat_r_t fat_create_path_entry(fat_create_ctx_t* c, fat_block_t* blk, const fat_mount_t* mnt) {
    ...
    c->found.valid = 0;
    if (!c->path) {
        FAT_CO_FAIL(c, blk, FS_ERR_BAD_ARGS);
    }
    ...
    if (!c->parent.found.valid) {
        FAT_CO_FAIL(c, blk, FS_ERR_NOT_FOUND);
    }
    ...
    if (c->scan.found.valid) {
        if (c->fail_if_exists) {
            FAT_CO_FAIL(c, blk, FS_ERR_EXISTS);
        }
...
fat_r_t fat_create_directory(fat_mkdir_ctx_t* m, fat_block_t* blk, const fat_mount_t* mnt) {
    ...
    if (!m->path) {
        FAT_CO_FAIL(m, blk, FS_ERR_BAD_ARGS);
    }
    ...
    if (!m->parent.found.valid) {
        FAT_CO_FAIL(m, blk, FS_ERR_NOT_FOUND);
    }
    ...
    if (m->root) {
        ...
        FAT_CO_FAIL(m, blk, FS_ERR_CORRUPT);
    }
...
fat_r_t fat_remove_path(fat_remove_ctx_t* r, fat_block_t* blk, const fat_mount_t* mnt,
    ...
    if (!r->path) {
        FAT_CO_FAIL(r, blk, FS_ERR_BAD_ARGS);
    }
    ...
    if (!r->resolve.found.valid) {
        FAT_CO_FAIL(r, blk, FS_ERR_NOT_FOUND);
    }
    ...
    if (r->is_rmdir) {
        ...
        if ((r->entry.attr & 0x10) == 0 || r->entry.cluster < 2) {
            FAT_CO_FAIL(r, blk, FS_ERR_NOT_DIR);
        }
        ...
        if (r->empty.result <= 0) {
            FAT_CO_FAIL(r, blk, FS_ERR_NOT_EMPTY);
        }
...
fat_r_t fat_op_readdir(fat_op_ctx_t* op, fat_block_t* blk, const fat_mount_t* mnt,
    ...
    if (mnt->root_entry_count == 0 || mnt->root_dir_sectors == 0) {
        ...
        FAT_CO_FAIL(s, blk, FS_ERR_NOT_FOUND);
    }
    ...
    for (;;) {
        for (; c->cur_sector < c->dir_sectors && c->entries_left > 0; ++c->cur_sector) {
            ...
            for (c->scan_index = 0; c->scan_index < c->entries_total; ++c->scan_index) {
                ...
                if (ent[0] == 0x00) {
                    fat_lfn_reset(&c->lfn);
                    FAT_CO_FAIL(c, blk, FS_ERR_NOT_FOUND); /* end-of-dir: miss */
                }
                if (ent[0] == 0xE5) {
                    fat_lfn_reset(&c->lfn);
                    continue;
                ...
                }
                {
                    ...
                    }
                    {
                        ...
                        if (cluster < 2) {
                            fat_lfn_reset(&c->lfn);
                            FAT_CO_FAIL(c, blk, FS_ERR_NOT_DIR);
                        }
                        c->root = 0;
                        c->cluster = cluster;
                        c->next = fat_chdir_next_component(c);
                        ...
                        /* Descend into the matched subdirectory and restart the
                         * scan against its sectors. */
