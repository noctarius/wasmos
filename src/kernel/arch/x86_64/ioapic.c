...
static inline void ioapic_write_sel(uint8_t reg) {
    *(volatile uint32_t*)(g_ioapic_base + IOAPIC_REG_SELECT) = reg;
}

static inline uint32_t ioapic_read_win(void) {
    return *(volatile uint32_t*)(g_ioapic_base + IOAPIC_REG_WINDOW);
}

static inline void ioapic_write_win(uint32_t val) {
...
static uint64_t find_xsdt_phys(const boot_info_t* boot_info) {
    uint64_t xsdt;
...
static void madt_parse(uint64_t xsdt_phys, uint64_t* out_ioapic_phys) {
        ...
        while (p < end) {
            ...
            if (e->type == 0u) {
                ...
                if ((proc->flags & MADT_PROC_FLAG_ENABLED) &&
                    proc->apic_id != (uint8_t)bsp_apic_id && g_cpu_count < WASMOS_MAX_CPUS) {
...
static void ioapic_map(uint64_t phys) {
...
static void ioapic_program_rtes(void) {
    for (uint32_t irq = 0; irq < 16u; irq++) {
        ...
        uint32_t lo =
            IOAPIC_RTE_MASK_BIT | IOAPIC_RTE_LEVEL | IOAPIC_RTE_ACTHI | IOAPIC_RTE_FIXED | vec;
