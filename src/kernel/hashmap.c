...
static inline void* node_value(hashmap_node_t* node) {
    return (void*)((uint8_t*)node + sizeof(hashmap_node_t));
}

/* Fibonacci/Knuth multiplicative hash, masked to the (power-of-two) table. */
static inline uint32_t hash_index(uint32_t key, uint32_t bucket_count) {
...
static hashmap_node_t** alloc_buckets(uint32_t count) {
    hashmap_node_t** b = (hashmap_node_t**)kmem_alloc((size_t)count * sizeof(hashmap_node_t*));
    if (b) {
...
static hashmap_node_t* find_node(hashmap_t* map, uint32_t key) {
