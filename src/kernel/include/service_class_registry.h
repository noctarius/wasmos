...
typedef void (*service_class_event_fn)(void *user,
...
int service_class_registry_add(const char *class_name, uint32_t instance,
                               uint32_t endpoint, uint32_t owner_ctx,
                               uint32_t pid);

/* Write matching providers to out[0..max) in ascending instance order; returns
 * the total match count (may exceed max). */
uint32_t service_class_registry_lookup(const char *class_name,
                                        service_class_provider_t *out,
                                        uint32_t max);

/* Subscribe notify_endpoint to class_name events. Idempotent per
 * (class_name, notify_endpoint). Returns 0 or negative on error. */
int service_class_registry_subscribe(const char *class_name,
                                      uint32_t notify_endpoint,
                                      uint32_t owner_ctx);
