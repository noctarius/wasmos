...
int kernel_ring3_spawn_native_probe(uint32_t parent_pid, uint32_t* out_pid) {
    ...
    if (process_spawn_as_parked(
            parent_pid, "ring3-native", ring3_probe_bootstrap_entry, 0, out_pid) != 0) {
        return -1;
    }
...
int kernel_ring3_spawn_thread_lifecycle_probe(uint32_t parent_pid, uint32_t* out_pid) {
    ...
    if (process_spawn_as_parked(
            parent_pid, "ring3-threading", ring3_probe_bootstrap_entry, 0, out_pid) != 0) {
        return -1;
    }
    proc = process_get(*out_pid);
    if (!proc) {
...
int kernel_ring3_spawn_fault_probe(uint32_t parent_pid, uint32_t* out_pid) {
    static const uint8_t code[] = {0xB8,
                                   ...
                                   0xEB,
                                   0xFE};
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault", code, (uint32_t)sizeof(code), out_pid);
}
...
int kernel_ring3_spawn_fault_write_probe(uint32_t parent_pid, uint32_t* out_pid) {
    static const uint8_t code[] = {0xB8,
                                   ...
                                   0xFE};
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-write", code, (uint32_t)sizeof(code), out_pid);
}
...
int kernel_ring3_spawn_fault_exec_probe(uint32_t parent_pid, uint32_t* out_pid) {
    static const uint8_t code[] = {0xB8,
                                   ...
                                   0xFE};
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-exec", code, (uint32_t)sizeof(code), out_pid);
}

int kernel_ring3_spawn_fault_ud_probe(uint32_t parent_pid, uint32_t* out_pid) {
    static const uint8_t code[] = {
        0xB8, 0x01, 0x00, 0x00, 0x00, 0xCD, 0x80, 0x0F, 0x0B, 0xEB, 0xFE};
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-ud", code, (uint32_t)sizeof(code), out_pid);
}
...
int kernel_ring3_spawn_fault_gp_probe(uint32_t parent_pid, uint32_t* out_pid) {
    ...
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-gp", code, (uint32_t)sizeof(code), out_pid);
}

int kernel_ring3_spawn_fault_de_probe(uint32_t parent_pid, uint32_t* out_pid) {
    static const uint8_t code[] = {
        0xB8, 0x01, 0x00, 0x00, 0x00, 0xCD, 0x80, 0x31, 0xD2, 0x31, 0xC0, 0xF7, 0xF2, 0xEB, 0xFE};
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-de", code, (uint32_t)sizeof(code), out_pid);
}
...
int kernel_ring3_spawn_fault_db_probe(uint32_t parent_pid, uint32_t* out_pid) {
    ...
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-db", code, (uint32_t)sizeof(code), out_pid);
}
...
int kernel_ring3_spawn_fault_bp_probe(uint32_t parent_pid, uint32_t* out_pid) {
    ...
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-bp", code, (uint32_t)sizeof(code), out_pid);
}

int kernel_ring3_spawn_fault_of_probe(uint32_t parent_pid, uint32_t* out_pid) {
    static const uint8_t code[] = {
        0xB8, 0x01, 0x00, 0x00, 0x00, 0xCD, 0x80, 0xCD, 0x04, 0x0F, 0x0B, 0xEB, 0xFE};
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-of", code, (uint32_t)sizeof(code), out_pid);
}

int kernel_ring3_spawn_fault_nm_probe(uint32_t parent_pid, uint32_t* out_pid) {
    static const uint8_t code[] = {
        0xB8, 0x01, 0x00, 0x00, 0x00, 0xCD, 0x80, 0xD9, 0xE8, 0x0F, 0x0B, 0xEB, 0xFE};
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-nm", code, (uint32_t)sizeof(code), out_pid);
}
...
int kernel_ring3_spawn_fault_ss_probe(uint32_t parent_pid, uint32_t* out_pid) {
    ...
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-ss", code, (uint32_t)sizeof(code), out_pid);
}
...
int kernel_ring3_spawn_fault_ac_probe(uint32_t parent_pid, uint32_t* out_pid) {
    ...
    return spawn_ring3_fault_probe_named(
        parent_pid, "ring3-fault-ac", code, (uint32_t)sizeof(code), out_pid);
}
