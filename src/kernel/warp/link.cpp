...
#define WASMOS_SYMBOLS(LINK)                                                                         \
    LINK("wasmos", "ipc_create_endpoint", warp_ipc_create_endpoint),                                 \
        ...
        LINK("wasmos", "io_in8", warp_io_in8), LINK("wasmos", "io_in16", warp_io_in16),              \
        LINK("wasmos", "io_in32", warp_io_in32), LINK("wasmos", "io_out8", warp_io_out8),            \
        LINK("wasmos", "io_out16", warp_io_out16), LINK("wasmos", "io_out32", warp_io_out32),        \
        ...
        LINK("wasmos", "phys_map", warp_phys_map), LINK("wasmos", "proc_info", warp_proc_info),      \
        ...
        LINK("wasmos", "irq_ack", warp_irq_ack), LINK("wasmos", "irq_unroute", warp_irq_unroute),    \
        ...
        LINK("wasmos", "env_get", warp_env_get), LINK("wasmos", "env_set", warp_env_set),            \
        LINK("wasmos", "env_unset", warp_env_unset), LINK("env", "abort", warp_env_abort),           \
        LINK("wasmos", "region_alloc", warp_region_alloc),                                           \
        LINK("wasmos", "irq_configure", warp_irq_configure),                                         \
        LINK("wasmos", "ipc_select_wait_timeout",                                                    \
             warp_ipc_select_wait_timeout), /* Appended at the tail to preserve positional hc_id ↔ \
                                             * HC_* enum mapping (WASMOS_SYMBOLS order defines the   \
                                             * ring-3 hostcall index). Keep in exact enum order:     \
                                             * HC_XFER_BUFFER_ACQUIRE(104), _UNBORROW(105),          \
                                             * HC_BUFFER_ACQUIRE(106), _UNBORROW(107),               \
                                             * HC_XFER_BUFFER_REBORROW(108),                         \
                                             * HC_BUFFER_REBORROW(109), HC_SPAWN_INFO_BUFFER(110),   \
                                             * HC_WASI_PROC_EXIT(111), HC_WASI_RANDOM_GET(112),      \
...
extern "C" uint32_t warp_ring3_dispatch(uint32_t hc_id, void* frame_ptr) {
    ...
    uint32_t dispatch_pid = process_current_pid();
    process_t* dispatch_proc = process_get(dispatch_pid);
    ...
    /* 2 */ case HC_IPC_SEND: {
        ...
        return warp_ipc_send((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, (uint32_t)a4,
                             (uint32_t)a5, s_a2, s_a3, s_ctx);
    }
    ...
    /* 28 */ case HC_XFER_BUFFER_READ:
        return warp_xfer_buffer_read((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, ctx5);
    /* 29 */ case HC_XFER_BUFFER_WRITE:
        return warp_xfer_buffer_write((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, ctx5);
    /* 30 */ case HC_BUFFER_BORROW:
        return warp_buffer_borrow((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, ctx5);
    /* 31 */ case HC_BUFFER_RELEASE:
        return warp_buffer_release((uint32_t)a0, (uint32_t)a1, ctx3);
    /* 106 */ case HC_BUFFER_ACQUIRE:
    ...
    /* 33 */ case HC_BLOCK_BUFFER_COPY:
        return warp_block_buffer_copy((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, ctx5);
    /* 34 */ case HC_BLOCK_BUFFER_WRITE:
        return warp_block_buffer_write((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3,
                                       ctx5);
    ...
    /* 59 */ case HC_INITFS_ENTRY_COPY:
        return warp_initfs_entry_copy((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, ctx5);
    /* 60 */ case HC_DMA_MAP_BORROW:
        return warp_dma_map_borrow((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, ctx5);
    /* 61 */ case HC_DMA_SYNC_BORROW:
        return warp_dma_sync_borrow((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, ctx5);
    /* 62 */ case HC_DMA_UNMAP_BORROW:
        return warp_dma_unmap_borrow((uint32_t)a0, ctx2);
    /* 63 */ case HC_PHYS_MAP:
    ...
    /* 65 */ case HC_PROC_INFO_EX:
        return warp_proc_info_ex((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, ctx5);
    /* 66 */ case HC_PROC_INFO_STATS:
        return warp_proc_info_stats((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3,
                                    (uint32_t)a4, ctx5);
    /* 67 */ case HC_XFER_BUFFER_BORROW:
        return warp_xfer_buffer_borrow((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, ctx4);
    /* 68 */ case HC_XFER_BUFFER_RELEASE:
        return warp_xfer_buffer_release((uint32_t)a0, ctx2);
    /* 104 */ case HC_XFER_BUFFER_ACQUIRE:
    ...
    /* 70 */ case HC_THREAD_CREATE:
        return warp_thread_create((uint32_t)a0, (uint32_t)a1, (uint32_t)a2, (uint32_t)a3, ctx5);
