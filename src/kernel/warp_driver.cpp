...
int warp_mem_ring3_map_jit(uint64_t user_root, uint8_t const* jit_ptr, size_t jit_size);
...
static void warp_r3_patch_basedata(uint8_t* linmem_kernel_ptr, uint64_t basedataLength,
                                   ...
                                   uint64_t jit_kernel_base, uint64_t jit_size, uint64_t custom_ctx,
                                   uint64_t runtime_ptr);
...
static int call_export_mod(vb::WasmModule* mod, const char* name, uint32_t argc,
    ...
    switch (argc) {
    case 0:
        mod->callExportedFunctionWithName<1>(k_stack_fence, name);
        break;
    case 1:
        mod->callExportedFunctionWithName<1>(k_stack_fence, name, argv[0]);
        break;
    case 2:
        mod->callExportedFunctionWithName<1>(k_stack_fence, name, argv[0], argv[1]);
        break;
    case 3:
...
static void warp_r3_patch_basedata(uint8_t* linmem_kernel_ptr, uint64_t basedataLength,
                                   ...
                                   uint64_t jit_kernel_base, uint64_t jit_size, uint64_t custom_ctx,
                                   uint64_t runtime_ptr) {
    if (!linmem_kernel_ptr || basedataLength == 0)
        return;
    ...
    auto patch_jit_va = [jit_kernel_base, jit_size](uint64_t* slot) {
        if (!slot || !*slot)
            return;
        uint64_t v = *slot;
        if (v < jit_kernel_base || v >= jit_kernel_base + jit_size)
            return;
        *slot = WARP_R3_JIT_BASE + (v - jit_kernel_base);
    };

    /* Write alloc-start user VA into the proxy slot (ring-3 stack bottom). */
    uint64_t* proxy_kernel = reinterpret_cast<uint64_t*>(stack_phys | kHalfBase);
...
static __attribute__((noinline)) void r3_do_iretq(uint64_t rip, uint64_t rsp, uint64_t rflags,
                                                  uint64_t rdi_v, uint64_t rsi_v, uint64_t rdx_v,
                                                  uint64_t rcx_v) {
...
static uint64_t warp_r3_resolve_export(vb::WasmModule* mod, const char* name) {
    ...
    {
        auto const raw_fn =
            mod->runtime_.getRawExportedFunctionByName(name_span, vb::Span<char const>{});
        kernel_fn = raw_fn.info().fncPtr();
    }
    ckpt->active = 0;

    vb::Span<uint8_t const> compiled = mod->getCompiledBinary();
    if (kernel_fn < compiled.data() || kernel_fn >= compiled.data() + compiled.size()) {
        return 0;
    }
...
static int warp_r3_call_export(vb::WasmModule* mod, const char* name, uint32_t argc,
    ...
    *reinterpret_cast<uint64_t*>(kstack + WARP_R3_STACK_SIZE - 8) = WARP_R3_RET_TRAMPOLINE;
...
int wasm_driver_start(wasm_driver_t* driver, const wasm_driver_manifest_t* manifest,
    ...
    klog_write("[warp-driver] start: ");
    klog_write(manifest->name ? manifest->name : "?");
    klog_write("\n");

    warp_heap_configure(driver->owner_pid, manifest->heap_size, 2ULL * 1024ULL * 1024ULL * 1024ULL);
...
int wasm_driver_endpoint(const wasm_driver_t* driver, uint32_t* out) {
    if (!driver || !out)
        return -1;
    *out = driver->endpoint;
    return 0;
...
#ifdef WASMOS_WASM_RUNTIME_WARP
    warp_driver_ring3_call_policy_t policy = warp_driver_ring3_call_policy_resolve(r3_root);
...
int wasm_driver_call(wasm_driver_t* driver, const char* name, uint32_t argc, uint32_t* argv) {
    if (!driver || !driver->active || !driver->wasm_module)
        return -1;
#ifdef WASMOS_WASM_RUNTIME_WARP
    ksync_spinlock_lock_noirq(&driver->lock);
...
int wasm_driver_call_unlocked(wasm_driver_t* driver, const char* name, uint32_t argc,
                              uint32_t* argv) {
    if (!driver || !driver->active || !driver->wasm_module)
        return -1;
    uint32_t prev = warp_runtime_enter(driver->owner_pid);
