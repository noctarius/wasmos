...
static int
xfer_buffer_state_validate_flags(uint32_t flags, uint32_t allowed_flags)
{
...
static void
xfer_buffer_state_clear_dma(xfer_buffer_state_t *state)
{
    state->dma_mapped = 0;
...
void
xfer_buffer_state_clear(xfer_buffer_state_t *state)
{
    if (!state) {
        return;
    }
    ...
    xfer_buffer_state_clear_dma(state);
}

int
xfer_buffer_state_borrow_from_source(xfer_buffer_state_t *state,
                                     uint32_t source_context_id,
                                     uint32_t flags,
                                     uint32_t allowed_flags)
{
    if (!state || source_context_id == 0 ||
        xfer_buffer_state_validate_flags(flags, allowed_flags) != 0) {
        return -1;
    }
    /* Current slot model allows only one active borrow record at a time. */
...
int
xfer_buffer_state_borrow_local(xfer_buffer_state_t *state,
                               uint32_t flags,
                               uint32_t allowed_flags)
{
    if (!state || xfer_buffer_state_validate_flags(flags, allowed_flags) != 0) {
        return -1;
    }
    if (state->borrow_active || state->dma_mapped) {
        return -1;
...
int
xfer_buffer_state_release(xfer_buffer_state_t *state)
{
    if (!state || !state->borrow_active || state->dma_mapped) {
        return -1;
    }
    xfer_buffer_state_clear(state);
    return 0;
}

void
xfer_buffer_state_drop_if_borrowed_from(xfer_buffer_state_t *state,
                                        uint32_t source_context_id)
{
    if (!state || source_context_id == 0) {
        return;
...
int
xfer_buffer_state_dma_map(xfer_buffer_state_t *state,
                          uint32_t source_context_id,
...
int
xfer_buffer_state_dma_sync(const xfer_buffer_state_t *state,
                           uint32_t offset,
                           uint32_t length)
{
...
int
xfer_buffer_state_dma_unmap(xfer_buffer_state_t *state,
                            uint32_t source_context_id)
{
    if (!state || !state->borrow_active || !state->dma_mapped) {
        return -1;
