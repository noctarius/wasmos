...
export class Runtime {
  ...
  asyncStart(coroutine: Coroutine, task: Task): Future | null {
    if (
      coroutine.state != CoroutineState.New &&
  ...
  then(
    ...
    if (
      continuation.active ||
      (future.runtime !== null && future.runtime !== this)
    ) {
      return null;
    }
    future.runtime = this;
  ...
  private groupStart(
    ...
    for (let i = 0; i < count; ++i) {
      ...
      if (
        continuations[i].active ||
        (input.runtime !== null && input.runtime !== this)
      ) {
        return null;
      }
    }
...
function continuationListRemoveFromFuture(
  future: Future,
  target: Continuation
): void {
...
function continuationListRemoveFromRuntime(
  runtime: Runtime,
  target: Continuation
): void {
  let prev: Continuation | null = null;
    ...
    if (group.kind == FutureGroupKind.Race) {
      ...
      if (!group.settled && group.completed + 1 == group.count) {
        ...
        group.promise.resolve(
          values !== null ? changetype<usize>(values) : 0
        );
        futureGroupAbandon(group);
      }
    }
