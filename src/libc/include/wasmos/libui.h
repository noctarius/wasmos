...
/* Prototypes for main renderer so component headers can call back (e.g. scroll_view children).
 * Component headers follow for full split of render (and future layout/event) ownership. */
...
ui_render_component_clip(ui_context_t *ctx, int32_t id, ui_rect_t clip, int32_t offset_y)
{
    ...
    /* TODO(libui-split): remaining per-type render (checkbox, list, dropdown, scroll, menus)
     * and their layout/event special cases live in this file for now.
     * Move them into their own libui_*.h the same way label/button were split. */
