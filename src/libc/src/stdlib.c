...
static size_t heap_align(size_t v) {
...
static int heap_grow_to(uint32_t need_end) {
...
static heap_block_t* heap_request_block(size_t payload_size) {
    if (payload_size > (size_t)(-1) - sizeof(heap_block_t))
        return NULL;
    const size_t total = heap_align(sizeof(heap_block_t) + payload_size);
    if (total > 0xFFFFFFFFu)
        return NULL;
    uint32_t start =
        (g_heap_cursor + (uint32_t)(sizeof(void*) - 1u)) & ~(uint32_t)(sizeof(void*) - 1u);
...
static void heap_split_block(heap_block_t* blk, size_t want) {
    if (!blk)
        return;
    const size_t aligned_want = heap_align(want);
    const size_t aligned_have = heap_align(blk->size);
    if (aligned_have <= aligned_want + sizeof(heap_block_t) + sizeof(void*))
        return;
