...
static void coroutine_enqueue(wasmos_wasm_runtime_t* runtime, wasmos_wasm_coroutine_t* coroutine) {
    coroutine->next = NULL;
    if (runtime->ready_tail) runtime->ready_tail->next = coroutine;
    else runtime->ready_head = coroutine;
...
static wasmos_wasm_coroutine_t* coroutine_dequeue(wasmos_wasm_runtime_t* runtime) {
    ...
    if (!runtime->ready_head) runtime->ready_tail = NULL;
    coroutine->next = NULL;
    return coroutine;
}
...
static void continuation_enqueue(wasmos_wasm_runtime_t* runtime,
    ...
    if (runtime->continuation_tail) runtime->continuation_tail->next = continuation;
    else runtime->continuation_head = continuation;
...
wasmos_future_t* wasmos_async_start(wasmos_wasm_runtime_t* runtime,
    ...
    if (!runtime || !coroutine || !resume ||
        ...
        return NULL;
    *coroutine = (wasmos_wasm_coroutine_t){.runtime = runtime, .resume = resume, .user = user,
                                            .state = WASMOS_WASM_COROUTINE_READY};
...
static bool promise_complete(wasmos_promise_t* promise, int32_t status, uintptr_t value) {
    ...
    wasmos_future_continuation_t* continuation;
    if (!promise || !(future = promise->future) || future->state != WASMOS_FUTURE_PENDING) return false;
    ...
    while (continuation) {
        ...
        if (continuation->active) continuation_enqueue(future->runtime, continuation);
        continuation = next;
    }
    return true;
}
...
bool wasmos_future_poll(const wasmos_future_t* future, int32_t* out_status, uintptr_t* out_value) {
    if (!future || future->state == WASMOS_FUTURE_PENDING) return false;
    if (out_status) *out_status = future->status;
    if (out_value) *out_value = future->value;
    return true;
}
...
int wasmos_wasm_coroutine_join(wasmos_wasm_coroutine_t* coroutine, int32_t* out_result) {
    ...
    if (status == 0 && out_result) *out_result = (int32_t)(intptr_t)value;
    return status;
}
...
static void continuation_dispatch(wasmos_future_continuation_t* continuation) {
    ...
    if (!future || future->state == WASMOS_FUTURE_PENDING) return;
    if (future->status == 0) {
        status = continuation->on_success ?
            continuation->on_success(continuation->user, future->value, &value) : 0;
        if (!continuation->on_success) value = future->value;
    } else {
        status = continuation->on_error ?
            continuation->on_error(continuation->user, future->status, &value) : future->status;
    }
    if (status == 0) (void)wasmos_promise_resolve(&continuation->child_promise, value);
    else (void)wasmos_promise_reject(&continuation->child_promise, status < 0 ? status : -1);
}

int wasmos_wasm_coroutine_run_budget(wasmos_wasm_runtime_t* runtime, size_t budget) {
    int resumed = 0;
    if (!runtime || runtime->running) return -1;
    ...
    for (;;) {
        ...
        if (budget != 0 && (coroutine = coroutine_dequeue(runtime)) != NULL) {
            ...
            if (status == WASMOS_WASM_TASK_YIELDED) {
                ...
                coroutine->state = WASMOS_WASM_COROUTINE_DEAD;
                if (status == 0) (void)wasmos_promise_resolve(&coroutine->completion_promise, value);
                else (void)wasmos_promise_reject(&coroutine->completion_promise, status);
            }
            continue;
        }
...
wasmos_future_t* wasmos_future_then(wasmos_wasm_runtime_t* runtime, wasmos_future_t* future,
    ...
    if (!runtime || !future || !continuation || continuation->active ||
        (future->runtime && future->runtime != runtime)) return NULL;
    future->runtime = runtime;
    *continuation = (wasmos_future_continuation_t){.future = future, .on_success = on_success,
        .on_error = on_error, .user = user, .active = true};
    ...
    } else continuation_enqueue(runtime, continuation);
...
static int32_t group_success(void* user, uintptr_t value, uintptr_t* out_value) {
    ...
    if (!group || !group->active) return -1;
...
static int32_t group_error(void* user, int32_t status, uintptr_t* out_value) {
    wasmos_future_continuation_t* continuation = user;
    wasmos_future_group_t* group = continuation ? continuation->group : NULL;
    if (!group || !group->active || status >= 0) return -1;
    if (!group->settled) {
        group->settled = true;
...
static wasmos_future_t* future_group_start(wasmos_wasm_runtime_t* runtime,
    ...
    if (!runtime || !group || !inputs || !continuations || count == 0 ||
        (kind == WASMOS_FUTURE_GROUP_ALL && !values)) return NULL;
    ...
    *group = (wasmos_future_group_t){.runtime = runtime, .values = values, .count = count,
