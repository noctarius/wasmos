...
static inline void wasmos_sha256_transform(wasmos_sha256_ctx_t* ctx, const uint8_t data[64]) {
    ...
    for (uint32_t i = 16; i < 64; ++i) {
        uint32_t s0 =
            wasmos_sha256_rotr(m[i - 15], 7) ^ wasmos_sha256_rotr(m[i - 15], 18) ^ (m[i - 15] >> 3);
        uint32_t s1 =
            wasmos_sha256_rotr(m[i - 2], 17) ^ wasmos_sha256_rotr(m[i - 2], 19) ^ (m[i - 2] >> 10);
    ...
    for (uint32_t i = 0; i < 64; ++i) {
        uint32_t S1 =
            wasmos_sha256_rotr(e, 6) ^ wasmos_sha256_rotr(e, 11) ^ wasmos_sha256_rotr(e, 25);
        uint32_t ch = (e & f) ^ ((~e) & g);
        uint32_t temp1 = h + S1 + ch + k[i] + m[i];
        uint32_t S0 =
            wasmos_sha256_rotr(a, 2) ^ wasmos_sha256_rotr(a, 13) ^ wasmos_sha256_rotr(a, 22);
...
static inline void wasmos_sha256_init(wasmos_sha256_ctx_t* ctx) {
    ctx->datalen = 0;
    ctx->bitlen = 0;
