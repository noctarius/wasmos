...
static inline void vring_mb(void) { __atomic_thread_fence(__ATOMIC_SEQ_CST); }
...
vring_layout(vring_t *vq, uint8_t *region_base, uint64_t region_phys,
             uint64_t region_bytes, uint16_t num, uint64_t align) {
    if (num == 0 || (num & (num - 1)) != 0) return -1;
    if (region_bytes < vring_size(num, align)) return -1;
    ...
    for (uint64_t i = 0; i < region_bytes; ++i) region_base[i] = 0;
...
static inline void
vring_set_notify(vring_t *vq, void (*notify)(void *user), void *user) {
    vq->notify = notify;
    vq->notify_user = user;
}
...
static inline int32_t
vring_alloc_desc(vring_t *vq, uint64_t buf_phys, uint32_t len, uint16_t flags) {
...
static inline void
vring_free_desc(vring_t *vq, uint16_t head) {
...
static inline void
vring_publish(vring_t *vq, uint16_t desc_head) {
...
static inline int32_t
vring_get_used(vring_t *vq, uint32_t *out_len) {
    if (!vring_has_used(vq)) return -1;
