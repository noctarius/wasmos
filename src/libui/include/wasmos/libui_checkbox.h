...
static inline void ui_render_checkbox(ui_context_t* ctx, const ui_component_t* c,
    ...
    ui_fill_rect_clip(ctx->mapped_base, ctx->width, ctx->height, bx, by, box, box, 0xFF2B3440u,
                      clip);
    ui_stroke_rect_clip(ctx->mapped_base, ctx->width, ctx->height, (ui_rect_t){bx, by, box, box}, 1,
                        0xFF9CB6CEu, clip);
    if (d && d->checked) {
        ui_fill_rect_clip(ctx->mapped_base, ctx->width, ctx->height, bx + 4, by + 4, box - 8,
                          box - 8, 0xFF66CC88u, clip);
    }
    ui_draw_text_clip(ctx, bx + box + 8, draw_bounds.y + (draw_bounds.h - ctx->font_px) / 2,
