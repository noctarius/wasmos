                      ...
                      0xFF172233u, clip);
    {
        ...
        if (d) {
            for (int32_t i = 0; i < d->list.count; ++i) {
                ...
                ui_fill_rect_clip(ctx->mapped_base, ctx->width, ctx->height, inner.x, row_y,
                                  content_w, item_h, row_bg, item_clip);
                for (int32_t level = 0; level < depth; ++level) {
                    const int32_t guide_x = inner.x + 10 + (level * 14);
                    ui_fill_rect_clip(ctx->mapped_base, ctx->width, ctx->height, guide_x, row_y + 3,
                                      1, item_h - 6, 0xFF3B516Au, item_clip);
                }
