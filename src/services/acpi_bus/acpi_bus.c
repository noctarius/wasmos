...
static int map_phys(uint64_t phys, uint32_t size) {
...
static const isa_id_entry_t* isa_lookup(uint8_t b2, uint8_t b3) {
...
static int aml_pkglen(const uint8_t* buf, uint32_t avail, uint32_t* out_len, uint32_t* consumed) {
    if (avail < 1) {
        return -1;
    }
    uint8_t b0 = buf[0];
...
static int aml_skip_int(const uint8_t* buf, uint32_t avail) {
    if (avail < 1) {
        return -1;
    }
    uint8_t op = buf[0];
...
static uint8_t irq_from_mask(uint16_t mask) {
...
static void scan_isa_devices(const uint8_t* aml, uint32_t aml_len, int32_t devmgr_ep,
                             int32_t src_ep, int32_t* req_id) {
    ...
    for (uint32_t i = 0; i + 5u <= aml_len; i++) {
        ...
        if (pos >= aml_len || aml_pkglen(aml + pos, aml_len - pos, &pkg_len, &consumed) != 0) {
            continue;
        }
        pos += consumed;
        if (pos >= aml_len) {
        ...
        for (uint32_t r = 0; r + 1u < res_end; r++) {
            ...
            if (tag == 0x47u && r + 8u <= res_end) {
                ...
                if (io_base == 0) {
                    io_base = (uint16_t)aml[pos + r + 2] | ((uint16_t)aml[pos + r + 3] << 8);
                }
                r += 7u;
                continue;
            }
            if ((tag == 0x22u || tag == 0x23u) && r + 2u < res_end) {
                ...
                uint16_t mask = (uint16_t)aml[pos + r + 1] | ((uint16_t)aml[pos + r + 2] << 8);
...
WASMOS_WASM_EXPORT int32_t initialize(int32_t proc_endpoint, int32_t ignored_arg1,
    ...
    int32_t devmgr_ep = wasmos_sys_svc_lookup_retry(proc_endpoint, src_ep, "devmgr.inv", 1, 1024);
    if (devmgr_ep < 0) {
        return -1;
    }
    int32_t req_id = 1;
    ...
    uint64_t sdt_phys = use_xsdt ? rsdp.xsdt_address : (uint64_t)rsdp.rsdt_address;
    ...
    for (uint32_t i = 0; i < n_entries; i++) {
        ...
        if (hdr->signature[0] == 'F' && hdr->signature[1] == 'A' && hdr->signature[2] == 'C' &&
            hdr->signature[3] == 'P') {
            facp_phys = ep;
            break;
        }
    ...
    (void)wasmos_ipc_send(devmgr_ep, src_ep, DEVMGR_ACPI_SCAN_DONE, req_id, 0, 0, 0, 0);
