...
static int cli_env_apply_export_line(const char* line) {
    ...
    int32_t nlen = 0;
    int32_t vlen = 0;
    if (cli_parse_name_value(
            line, name, (int)sizeof(name), value, (int)sizeof(value), &nlen, &vlen) != 0) {
        return -1;
    }
    (void)nlen;
    (void)vlen;
...
static int cli_vt_call(int32_t msg_type, int32_t arg0, int32_t arg1, int32_t arg2, int32_t arg3,
    ...
    int32_t req_id = 0;
    wasmos_future_t* future = wasmos_sys_wasm_ipc_future_send(&g_loop,
                                                              &op,
                                                              g_vt_endpoint,
                                                              g_vt_client_endpoint,
                                                              msg_type,
...
static int cli_spawn_exec_path(const char* input, int32_t* out_pid) {
    ...
    *out_pid = -1;
    ...
    if (wasmos_xfer_buffer_write(bid, resolved, (int32_t)path_len, 0) != 0) {
        (void)wasmos_xfer_buffer_release(bid);
        return -1;
    }
    if (args_len > 0u &&
        wasmos_xfer_buffer_write(bid, args, (int32_t)args_len, (int32_t)write_off) != 0) {
        (void)wasmos_xfer_buffer_release(bid);
        return -1;
    }
    if (cli_send_proc(
            PROC_IPC_SPAWN_PATH, 0, ((uint32_t)bid << 12) | (path_len & 0xFFFu), args_len, 0) !=
        0) {
        (void)wasmos_xfer_buffer_release(bid);
        return -1;
    }
