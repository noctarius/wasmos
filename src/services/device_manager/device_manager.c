            ...
            int32_t max_empty_polls)
{
    ...
    for (;;) {
        int32_t ready_ep = -1;
        if (g_dm_call_select_id < 0) {
            return -1;
        }
        ready_ep = wasmos_ipc_select_wait(g_dm_call_select_id);
        if (ready_ep < 0) {
            return -1;
        }
        if (ready_ep == source_endpoint) {
            int32_t recv_rc = wasmos_ipc_drain(source_endpoint);
            if (recv_rc < 0) {
                return -1;
            }
            if (recv_rc == 0) {
                if (max_empty_polls > 0 && ++empty_polls >= max_empty_polls) {
                    return -1;
                }
                continue;
            }
        ...
        if (ready_ep == g_dm.query_endpoint) {
            int32_t handled = wasmos_sys_event_loop_poll(&g_dm_query_loop, 16);
            if (handled < 0) {
                return -1;
            }
            empty_polls = 0;
...
count_loaded_active_rules(void)
{
    ...
    for (uint32_t i = 0; i < g_dm.acpi_match_rule_count; ++i) {
        if (g_dm.acpi_match_rules[i].active) {
            count++;
        }
    }
    return count;
...
poll_boot_rules_async(void)
{
    ...
    if (g_dm.acpi_match_rule_count > 0) {
        console_write("[device-manager] boot rules loaded acpi_match\n");
...
load_rules_if_available(void)
{
    if (!g_dm.rules_init_loaded) {
        ...
        if (read_len >= 0) {
            ...
            if (g_dm.acpi_match_rule_count > 0) {
                console_write("[device-manager] init rules loaded acpi_match\n");
                queue_acpi_match_rule_spawns();
            }
...
queue_acpi_match_rule_spawns(void)
{
    ...
    for (uint32_t ri = 0; ri < g_dm.acpi_match_rule_count; ++ri) {
        ...
        for (uint32_t di = 0; di < g_dm.registry_count; ++di) {
             ...
             * irq_hint = IRQ line. */
            g_dm.active_rule_spawn_caps.cap_flags = DEVMGR_CAP_IO_PORT | DEVMGR_CAP_IRQ;
            g_dm.active_rule_spawn_caps.io_port_min = rec->device_id;
...
dm_handle_inventory_message(void *user, const wasmos_ipc_message_t *msg)
{
    ...
    if (msg->type == DEVMGR_ACPI_SCAN_DONE) {
...
consume_acpi_inventory(void)
{
    ...
    g_dm_acpi_scan_done = 0;
    console_write("[device-manager] waiting for acpi-bus inventory\n");
    while (!g_dm_acpi_scan_done) {
        int32_t handled = wasmos_sys_event_loop_poll(&g_dm_inventory_loop, 16);
        if (handled < 0) {
            console_write("[device-manager] acpi inventory recv failed\n");
            break;
        }
        if (handled == 0) {
            (void)wasmos_sched_yield();
    ...
    if (g_dm_acpi_scan_done) {
        console_write("[device-manager] acpi-bus scan complete\n");
    }
    /* ACPI device matching is handled by acpi_match_rules loaded from the
     * boot rules file.  queue_acpi_match_rule_spawns() is called after
...
handle_query_endpoint_nonblocking(void)
{
    for (int32_t i = 0; i < 16; ++i) {
        ...
        if (wasmos_ipc_drain(g_dm.query_endpoint) <= 0) {
            return;
        }
        wasmos_ipc_message_read_last(&msg);
...
consume_inventory_events_nonblocking(void)
{
    for (int32_t i = 0; i < 16; ++i) {
        ...
        if (wasmos_ipc_drain(g_dm.inventory_endpoint) <= 0) {
            return;
        }
        wasmos_ipc_message_read_last(&msg);
        dm_handle_inventory_message(0, &msg);
    }
}
...
drain_reply_endpoint_nonblocking(int32_t budget)
{
    ...
    for (int32_t i = 0; i < budget; ++i) {
        ...
        if (wasmos_ipc_drain(g_dm.reply_endpoint) <= 0) {
            return;
        }
        wasmos_ipc_message_read_last(&msg);
        dm_handle_ipc_default(0, &msg);
    }
}
           ...
           int32_t ignored_arg3)
{
    ...
    g_dm_call_select_id = wasmos_ipc_select_create();
    if (g_dm_call_select_id < 0 ||
        wasmos_ipc_select_add(g_dm_call_select_id, g_dm.reply_endpoint) != 0 ||
        wasmos_ipc_select_add(g_dm_call_select_id, g_dm.query_endpoint) != 0) {
        console_write("[device-manager] combined select init failed\n");
        wasmos_sys_ipc_recv_loop();
    }
    ...
    g_dm.need_acpi_bus = (g_dm.acpi_bus_index >= 0) ? 1 : 0;
    g_dm.need_fat = 0;
    g_dm.need_fs_init = 0;
    ...
    for (;;) {
        ...
        if (g_dm.phase == HW_PHASE_SPAWN) {
            ...
            if (target == HW_SPAWN_RULE_PATH) {
                ...
                if (rc == 0) {
                    ...
                    } else if (g_dm.active_rule_spawn_kind == RULE_SPAWN_KIND_ACPI_MATCH) {
                        if (g_dm.active_rule_spawn_index >= 0 &&
        ...
        if (g_dm.phase == HW_PHASE_IDLE) {
            ...
            drain_reply_endpoint_nonblocking(4);
