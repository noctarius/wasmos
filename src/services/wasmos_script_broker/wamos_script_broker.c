...
static int32_t script_request_string_at(int32_t buffer_id, uint32_t offset, uint32_t len,
                                        char* out_text, uint32_t out_text_size) {
    ...
    if (wasmos_xfer_buffer_read(buffer_id, out_text, (int32_t)len, (int32_t)offset) != 0) {
        return -1;
    }
    out_text[len] = '\0';
    return 0;
...
static int32_t script_read_interpreter(int32_t buffer_id, uint32_t blob_offset, uint32_t blob_size,
                                       char* out_interp, uint32_t out_interp_size) {
    char head[64];
    ...
    if (wasmos_xfer_buffer_read(buffer_id, head, (int32_t)copy_len, (int32_t)blob_offset) != 0) {
        return -1;
    }
    head[copy_len] = '\0';
    if (head[0] != '#' || head[1] != '!') {
...
static int32_t script_plan_write_string(int32_t buffer_id, uint32_t* io_offset, const char* text,
    ...
    if (wasmos_xfer_buffer_write(buffer_id, text, (int32_t)len, (int32_t)*io_offset) != 0) {
        return -1;
    }

    static const char nul = '\0';
    if (wasmos_xfer_buffer_write(buffer_id, &nul, 1, (int32_t)(*io_offset + len)) != 0) {
        return -1;
    }
...
static int32_t script_register(int32_t proc_endpoint, int32_t broker_endpoint) {
    ...
    for (;;) {
        ...
        if (msg.arg1 < (int32_t)sizeof(request) || msg.arg0 < 0) {
            ...
            (void)wasmos_ipc_send(msg.source,
                                  ...
                                  msg.request_id,
                                  0,
                                  WASMOS_ERR_PROC_SPAWN_BROKER_PLAN,
                                  0,
                                  0);
            continue;
        }
        ...
        if (wasmos_xfer_buffer_read(msg.arg2, &request, (int32_t)sizeof(request), msg.arg0) != 0) {
            ...
            (void)wasmos_ipc_send(msg.source,
                                  ...
                                  msg.request_id,
                                  0,
                                  WASMOS_ERR_PROC_SPAWN_BROKER_IPC,
                                  0,
                                  0);
            continue;
        }
        ...
        if (script_request_string_at(
                msg.arg2, request.path_offset, request.path_len, guest_path, sizeof(guest_path)) !=
            0) {
        ...
        if (script_read_interpreter(
                msg.arg2, request.blob_offset, request.blob_size, interp, sizeof(interp)) != 0) {
        ...
        if (wasmos_xfer_buffer_write(msg.arg2, &plan, (int32_t)sizeof(plan), 0) != 0 ||
            script_plan_write_string(
                msg.arg2, &plan_cursor, SCRIPT_HOST_PATH, &path_offset, &path_len) != 0 ||
            script_plan_write_string(msg.arg2, &plan_cursor, guest_path, &args_offset, &args_len) !=
                0) {
