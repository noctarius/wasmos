...
int main(void) {
    ...
    int connected =
        use_tls ? wasmos_net_tls_connect(
                      &sock, stack_ep, reply_ep, addr, (uint16_t)u.port, RING_CAP, rid, u.host)
                : wasmos_net_tcp_connect(
                      &sock, stack_ep, reply_ep, addr, (uint16_t)u.port, RING_CAP, rid);
    if (connected != 0) {
