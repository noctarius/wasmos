...
@external("wasmos", "sched_cpu_count") declare function sched_cpu_count(): i32;
...
export function main(_args: Array<string>): i32 {
    ...
    for (let c: i32 = 0; c < ncpus; ++c) {
        ...
        const row = padLeft(c.toString(), 4) + "  " + padLeft(ready.toString(), 5) + "  " +
                    padLeft(running.toString(), 11) + "  " + padLeft(last.toString(), 9) + "  " +
                    padLeft(steals.toString(), 6) + "  " + padLeft(dispatched.toString(), 10);
        std.println(row);
    }
