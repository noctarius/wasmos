...
class IrqRouteCapabilityTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cfg = default_config()
    ...
    def test_irq_route_capability_deny_then_allow(self):
        ...
        self._cmd_expect("exec irq-route-deny", [b"spawned pid", b"irq-route-deny: ok"])
        self._cmd_expect("exec irq-route-allow", [b"spawned pid", b"irq-route-allow: ok"])
