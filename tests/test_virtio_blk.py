...
class VirtioBlkTest(unittest.TestCase):
    ...
    def test_every_backend_appears_as_its_own_disk(self) -> None:
        ...
        The instance encodes (backend << 8) | unit. ATA's drives are 256, 257 and
        258 because its units are drive numbers; the virtio disk's unit comes
        from its PCI slot, so its instance is checked by decoding rather than by
        a literal. Both are derived from what the disk IS rather than handed out
        ...
        self.session.send("blkinfo")
        self.assertTrue(
            self.session.expect(b"[blkinfo] providers=4", timeout_s=60),
            "the block class did not hold all four disks (ATA's three drives plus "
        ...
        for marker in (
            ...
            b"instance=258 driver=ata unit=2",
        ):
            self.assertTrue(
    ...
    def test_block_class_read_returns_the_disk_signature(self) -> None:
        ...
        self.assertTrue(
            self.session.expect(b"[blkinfo] providers=4", timeout_s=60),
            'the "block" class did not hold all four disks — a backend did not '
