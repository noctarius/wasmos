...
import { Box, Coroutine, Future, Promise, Runtime, Task, AWAIT_PENDING, TASK_COMPLETE, TASK_YIELDED } from "./coroutine";
...
class SumTwoByHand extends Task {
  ...
  constructor(public a: Future, public b: Future) {
    super();
  }
...
function testMatchesTheHandWrittenTask(): i32 {
  ...
  for (let variant = 0; variant < 2; ++variant) {
    ...
    const b = new Pair();
    const coroutine = new Coroutine();
    const task: Task = variant == 0 ? <Task>sumTwo(a.future, b.future)
