...
static int test_yield_await_and_join(void) {
    static const uint32_t expected[] = {1u, 2u, 3u, 4u, 5u, 6u};
    ...
    if (run_count != 8 || state.event_count != sizeof(expected) / sizeof(expected[0]) ||
    ...
    if (wasmos_native_coroutine_run(&state.runtime) != 0) {
        return __LINE__;
    }
    return 0;
}
...
static int test_rejection_and_poll(void) {
    ...
    if (wasmos_native_future_poll(&future, &status, &value) ||
