...
static void
device_complete_one(vring_t *vq, uint16_t avail_slot, uint32_t wrote) {
...
test_size_and_layout(void) {
    ...
    if (vring_size(QNUM, QALIGN) != expect) return __LINE__;

    vring_t vq;
    if (vring_layout(&vq, g_region, 0x1000000ULL, sizeof(g_region), QNUM, QALIGN) != 0)
        return __LINE__;
    if (vq.num != QNUM) return __LINE__;
    if (vq.num_free != QNUM) return __LINE__;
    if (vq.last_used_idx != 0) return __LINE__;
    if (*vq.avail_idx != 0 || *vq.used_idx != 0) return __LINE__;
    /* desc/avail/used must all sit inside the region. */
    if ((uint8_t *)vq.desc < g_region) return __LINE__;
    if ((uint8_t *)vq.used_ring + QNUM * 8u > g_region + sizeof(g_region)) return __LINE__;
    /* used ring must be align-aligned relative to the region base. */
    if ((((uint8_t *)vq.used_flags - g_region) % QALIGN) != 0) return __LINE__;
    return 0;
}
...
test_reject_bad_params(void) {
    ...
    if (vring_layout(&vq, g_region, 0, sizeof(g_region), 3, QALIGN) == 0) return __LINE__;
    /* region too small. */
    if (vring_layout(&vq, g_region, 0, 8, QNUM, QALIGN) == 0) return __LINE__;
    return 0;
}

static int
test_publish_kick_consume(void) {
    vring_t vq;
    ...
    if (d0 < 0 || d1 < 0) return __LINE__;
    if (vq.num_free != QNUM - 2) return __LINE__;
    if (vq.desc[d0].addr != 0x2000000ULL + 0x100) return __LINE__;
    if (vq.desc[d0].len != 2048) return __LINE__;
    if ((vq.desc[d0].flags & VRING_DESC_F_WRITE) == 0) return __LINE__;

    vring_publish(&vq, (uint16_t)d0);
    vring_publish(&vq, (uint16_t)d1);
    if (*vq.avail_idx != 2) return __LINE__;
    if (vq.avail_ring[0] != (uint16_t)d0 || vq.avail_ring[1] != (uint16_t)d1) return __LINE__;

    if (vring_has_used(&vq)) return __LINE__;   /* nothing completed yet */
    vring_kick(&vq);
    if (g_notify_calls != 1) return __LINE__;

    /* Device completes the first buffer with 1500 bytes. */
    device_complete_one(&vq, 0, 1500);
    if (!vring_has_used(&vq)) return __LINE__;
    uint32_t len = 0;
    int32_t got = vring_get_used(&vq, &len);
    if (got != d0) return __LINE__;
    if (len != 1500) return __LINE__;
    if (vring_has_used(&vq)) return __LINE__;   /* only one was completed */

    /* Recycle the consumed descriptor. */
    uint16_t before = vq.num_free;
    vring_free_desc(&vq, (uint16_t)got);
    if (vq.num_free != before + 1) return __LINE__;

    /* Device completes the second buffer. */
    device_complete_one(&vq, 1, 64);
    got = vring_get_used(&vq, &len);
    if (got != d1 || len != 64) return __LINE__;
    return 0;
}

static int
test_free_list_exhaustion(void) {
    vring_t vq;
    if (vring_layout(&vq, g_region, 0, sizeof(g_region), QNUM, QALIGN) != 0) return __LINE__;
    for (uint16_t i = 0; i < QNUM; ++i) {
        if (vring_alloc_desc(&vq, 0x1000ULL * (i + 1), 16, 0) < 0) return __LINE__;
    }
    if (vq.num_free != 0) return __LINE__;
    if (vring_alloc_desc(&vq, 0xdead, 16, 0) != -1) return __LINE__;  /* full */
    /* Free one and confirm we can allocate again. */
    vring_free_desc(&vq, 3);
    if (vring_alloc_desc(&vq, 0xbeef, 16, 0) < 0) return __LINE__;
    return 0;
}

static int
test_consumer_validation(void) {
    vring_t vq;
    if (vring_layout(&vq, g_region, 0, sizeof(g_region), QNUM, QALIGN) != 0) return __LINE__;
    ...
    *vq.used_idx = 1;
    uint32_t len = 12345;
    int32_t got = vring_get_used(&vq, &len);
    if (got != -1) return __LINE__;                 /* rejected, not trusted */
    if (vq.last_used_idx != 1) return __LINE__;     /* but still advanced past it */
    return 0;
}
...
main(void) {
    int rc;
    if ((rc = test_size_and_layout()) != 0) return rc;
    if ((rc = test_reject_bad_params()) != 0) return rc;
    if ((rc = test_publish_kick_consume()) != 0) return rc;
    if ((rc = test_free_list_exhaustion()) != 0) return rc;
    if ((rc = test_consumer_validation()) != 0) return rc;
    return 0;
}
