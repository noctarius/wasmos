...
#include "xfer_buffer/state.h"

static void
test_borrow_from_source_rejects_second_active_borrow(void)
{
    xfer_buffer_state_t state = {0};

    assert(xfer_buffer_state_borrow_from_source(&state, 41u, 0x1u, 0x3u) == 0);
    assert(state.borrow_active == 1u);
    assert(state.borrow_source_context_id == 41u);
    assert(xfer_buffer_state_borrow_from_source(&state, 77u, 0x2u, 0x3u) != 0);
    assert(state.borrow_source_context_id == 41u);
...
test_release_requires_active_borrow(void)
{
    ...
    assert(xfer_buffer_state_release(&state) != 0);
...
test_release_rejects_dma_mapped_borrow(void)
{
    ...
    assert(xfer_buffer_state_borrow_from_source(&state, 9u, 0x3u, 0x3u) == 0);
    assert(xfer_buffer_state_dma_map(&state, 9u, 4096u, 16u, 32u, 0x3u) == 0);
    assert(xfer_buffer_state_release(&state) != 0);
    assert(state.borrow_active == 1u);
...
test_drop_if_borrowed_from_clears_borrow_and_dma_state(void)
{
    ...
    assert(xfer_buffer_state_borrow_from_source(&state, 19u, 0x3u, 0x3u) == 0);
    assert(xfer_buffer_state_dma_map(&state, 19u, 4096u, 64u, 128u, 0x1u) == 0);
    xfer_buffer_state_drop_if_borrowed_from(&state, 19u);
    assert(state.borrow_active == 0u);
...
test_dma_unmap_keeps_borrow_until_release(void)
{
    ...
    assert(xfer_buffer_state_borrow_from_source(&state, 33u, 0x1u, 0x3u) == 0);
    assert(xfer_buffer_state_dma_map(&state, 33u, 4096u, 128u, 64u, 0x1u) == 0);
    assert(xfer_buffer_state_dma_unmap(&state, 33u) == 0);
    assert(state.borrow_active == 1u);
...
main(void)
{
    ...
    printf("test_xfer_buffer_state: ok\n");
    return 0;
}
