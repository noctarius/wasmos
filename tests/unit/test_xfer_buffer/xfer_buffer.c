...
static void test_acquire_validation(test_stats_t* stats) {
    ...
    check(stats,
          xfer_buffer_acquire(
              BUFFER_KIND_FRAMEBUFFER, 13u, framebuffer_size + 1u, &framebuffer_owner) != 0,
...
static void test_get_owned_and_release_owned_validation(test_stats_t* stats) {
    ...
    check(
        stats, xfer_buffer_release_owned(&owner) == 0, "release_owned accepts real owner binding");
    check(stats,
          xfer_buffer_get_owned(&owner.buffer, 20u, &fetched) != 0,
...
static void test_borrow_validation_and_owner_access(test_stats_t* stats) {
    ...
    check(
        stats, xfer_buffer_borrow(&owner, 41u, 0x4u, &borrow) != 0, "borrow rejects invalid flags");
    check(stats,
...
static void test_release_owned_cascade_revokes_borrows(test_stats_t* stats) {
    ...
    check(
        stats, xfer_buffer_unborrow(&borrow) != 0, "borrow handle is stale after release cascade");
    check(stats,
...
static void test_unborrow_lender_authority(test_stats_t* stats) {
    ...
    check(stats,
          xfer_buffer_get_lent(borrow.borrow_id, 80u, &resolved) == WASMOS_ERR_NONE,
    ...
    check(stats,
          xfer_buffer_get_lent(reborrow.borrow_id, 81u, &resolved) == WASMOS_ERR_NONE,
...
static void test_owner_side_dma_contract(test_stats_t* stats) {
    ...
    check(stats,
          xfer_buffer_dma_map_owned(
              &owner, owner.buffer.size_bytes, 1u, WASMOS_DMA_DIR_TO_DEVICE, &mapping) != 0,
...
static void test_borrow_dma_contract(test_stats_t* stats) {
    ...
    check(stats,
          xfer_buffer_dma_map_borrow(
              &write_borrow, 0u, 64u, WASMOS_DMA_DIR_FROM_DEVICE, &mapping) == 0,
...
static void test_acquire_variable_size(test_stats_t* stats) {
    ...
    for (uint32_t i = 0; i < sizeof(requests) / sizeof(requests[0]); ++i) {
        ...
        check(
            stats, ok && (owner.buffer.size_bytes % 4096u) == 0u, "transfer size is page-aligned");
        if (ok) {
...
static void test_acquire_capacity_semantics(test_stats_t* stats) {
    ...
    check(stats,
          xfer_buffer_acquire(
              BUFFER_KIND_FRAMEBUFFER, 201u, framebuffer_size, &framebuffer_owner) == 0,
...
static void test_borrow_rights_matrix(test_stats_t* stats) {
    ...
    check(stats,
          xfer_buffer_borrow(
              &owner, 213u, BUFFER_BORROW_READ | BUFFER_BORROW_WRITE | 0x4u, &second) != 0,
    ...
    check(
        stats, borrow_a.borrow_id != borrow_b.borrow_id, "fan-out borrows get distinct borrow ids");
    check(stats,
          xfer_buffer_unborrow(&borrow_a) == 0,
...
static void test_reborrow_extended(test_stats_t* stats) {
    ...
    check(stats,
          xfer_buffer_borrow(
              &owner, 281u, BUFFER_BORROW_READ | BUFFER_BORROW_WRITE, &rw_upstream) == 0,
...
static void test_owner_dma_extended(test_stats_t* stats) {
    ...
    check(stats,
          xfer_buffer_dma_map_owned(&owner, 0u, 64u, WASMOS_DMA_DIR_TO_DEVICE, &second) != 0,
...
static void test_borrow_dma_extended(test_stats_t* stats) {
    ...
    check(stats,
          xfer_buffer_dma_map_borrow(
              &read_borrow, 0u, size + 1u, WASMOS_DMA_DIR_TO_DEVICE, &mapping) != 0,
          "dma_map_borrow rejects length larger than the object");
    check(stats,
          xfer_buffer_dma_map_borrow(
              &read_borrow, size - 64u, 128u, WASMOS_DMA_DIR_TO_DEVICE, &mapping) != 0,
...
static void test_dma_sync_unmap_negatives(test_stats_t* stats) {
    ...
    check(
        stats, xfer_buffer_dma_sync(&empty, 0u, 16u) != 0, "dma_sync rejects an inactive mapping");
    check(stats, xfer_buffer_dma_unmap(&empty) != 0, "dma_unmap rejects an inactive mapping");

    check(stats,
          xfer_buffer_dma_map_owned(&owner, 0u, 128u, WASMOS_DMA_DIR_BIDIR, &mapping) == 0,
...
static void test_object_phys(test_stats_t* stats) {
    ...
    check(
        stats, xfer_buffer_object_phys(&nul) == 0u, "object_phys returns 0 for a zero descriptor");
